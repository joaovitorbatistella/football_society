<template>
  <v-container align-center justify-center :style="{
    ...
    <v-row >
      <v-col justify="center" align="center" cols="12" lg="4" md="4" :style="{ 
        }"
      >
        <v-row :style="{ minHeight: '40vh', maxHeight: '40vh' }">
          <v-layout align-center justify-content-center column>
            ...
            <v-card>
              <v-card-title> {{ formattedDate(lastGamesList[0].data_hora) }} </v-card-title>
              <v-card-text>
                <span>{{ lastGamesList[0].nome_cliente }} |</span>
                <span> R$ {{ lastGamesList[0].valor }}</span>
              </v-card-text>
            </v-card>
          </v-layout>
        </v-row>
      ...
      <v-col justify="center" align="center" cols="12" lg="8" md="8" :style="{
          padding: '50px'
        }"
      >
        <v-data-table
  ...
  methods: {
      ...
      switch (this.searchOptionSelected.id) {
        case 1:
          const configDateGame = {
            ...
            console.log(configDateGame)
            await this.$axios
              .get(`game/list/`, configDateGame)
              .then(({ data }) => {
                this.gamesList = []
        ...
        case 2:
          const config = {
            headers: {
              ...
              params: {
                attendanceId: this.attendanceSelected.codigo
              }
            };
            console.log(config)
            await this.$axios
    ...
    async updateTable() {
      try {
        ...
        await this.$axios
          .get(`game/list/`, configLast)
          .then(({ data }) => {
            this.lastGamesList = []
            this.lastGamesList = data.response
          ...
          await this.$axios
            .get(`attendance/list/`, configAttendance)
            .then(({ data }) => {
