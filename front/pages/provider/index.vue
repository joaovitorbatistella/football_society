        ...
        >
          <template v-slot:top>
            ...
            >
                  ...
                  <form>
                    ...
                    >
                      ...
                      <v-text-field
                        ...
                        label="Informe o nome do fornecedor, ou parte dele:"
                        solo
                        dense
                      ></v-text-field>
                      <v-select
                      ...
                      >
                        <template v-slot:item="{ on, item }">
                          <v-list-item v-on="on" :nome="item.nome">{{ item.nome + ' - ' + item.uf }}</v-list-item>
                        </template> 
                      </v-select>
                      ...
                      <v-row>
                        ...
                        >
                          <v-select
                            ...
                            item-value="codigo"
                            return-object
                            label="Selecione a cidade do fornecedor:"
                            small-chips
                            dense
                            dark
                            clearable
              ...
              v-model="dialogDelete" max-width="500px">
                <v-card>
                  <v-card-title class="text-h5">Você deseja excluir este fornecedor?</v-card-title>
                  <v-card-actions>
                    <v-spacer></v-spacer>
          ...
          <template v-slot:item.actions="{ item }">
            <v-icon
              small
              class="mr-2"
              @click="editItem(item.cod_fornecedor)"
            >
              mdi-pencil
            </v-icon>
            <v-icon
              small
              @click="deleteItem(item.cod_fornecedor)"
            >
              mdi-delete
            </v-icon>
          </template>
...
export default {
  ...
  data: () => ({
    ...
    heading: 'Relatorio de Fornecedores',
    searchOptions: [
      { id: 1, value: 'Cod.' },
      { id: 2, value: 'Nome' },
      { id: 3, value: 'Cidade' },
  ...
  methods: {
    async search() {
      ...
      switch (this.searchOptionSelected.id) {
        case 1:
          const configIdProvider = {
            headers: {
              Authorization: 'Bearer '+ token
              },
              params: {
                id: parseInt(this.searchId)
              }
            };
            console.log(configIdProvider)
            await this.$axios
              .get(`provider/list/`, configIdProvider)
              .then(({ data }) => {
                this.providersList = []
        ...
        case 2:
          const configNameProvider = {
            ...
            console.log(configNameProvider)
            await this.$axios
              .get(`provider/list/`, configNameProvider)
              .then(({ data }) => {
                this.providersList = []
                this.providersList = data.response   
              })
        ...
        case 3:
          const configCityProvider = {
            headers: {
              Authorization: 'Bearer '+ token
              },
              params: {
                cityId: this.citySelected.codigo
              }
            };
            console.log(configCityProvider)
            await this.$axios
              .get(`provider/list/`, configCityProvider)
              .then(({ data }) => {
                this.providersList = []
                this.providersList = data.response   
              })
    ...
    generatePDF() {
      const columns = [
        { title: "Código", dataKey: "cod_fornecedor" },
    ...
    async editItem (key) {
      try {
        ...
        this.loading = true
        await this.$axios
          .get(`provider/list`, configProvider)
          .then(({ data }) => {
        ...
        console.log("configCity", configCity)
    ...
    async deleteItemConfirm (key) {
      ...
      await this.$axios
        .delete(`provider/delete/${key}`, {headers})
        .then(({data}) => {
          console.log(data)
          if(data.type == 'success') {
            ...
            alert('Este fornecedor possui produtos referenciados a sí, delete-os e tente novamente!')
            this.closeDelete()
            return;
          }
        })
    ...
    save () {
      if (this.editedIndex > -1) {
        ...
        this.loading = true
        this.$axios
          .put(`provider/update/${this.editingProvider}`, data, {headers})
          .then(({ data }) => {
            console.log(data)
            this.updateTable()
          })
