      ...
      >
        ...
        >
                  ...
                  <form>
                    ...
                    >
                      ...
                      <v-text-field
                        color="lime accent-3"
                        v-model="searchEmail"
                        v-if="searchOptionSelected.id == 4"
                        :style="{
                          marginRight: '8px',
                          maxWidth: '180px'
                        }"
          ...
          <template v-slot:item.dt_nascimento="{ item }">
            <span>{{ formattedDate(item.dt_nascimento) }}</span>
          </template>
          <template v-slot:item.actions="{ item }">
            <v-icon
              small
              class="mr-2"
              @click="editItem(item.cod_cliente)"
            >
              mdi-pencil
            </v-icon>
            <v-icon
              small
              @click="deleteItem(item.cod_cliente)"
            >
              mdi-delete
            </v-icon>
          </template>
...
export default {
  ...
  data: () => ({
    ...
    sexList: [
      {nome: 'Masculino', sex: 'M'},
      {nome: 'Feminino', sex: 'F'},
      {nome: 'Indefinido', sex: 'U'},
    ],
    cityList: []
  }),
  async beforeMount() {
  ...
  methods: {
      ...
      switch (this.searchOptionSelected.id) {
        case 1:
          const configIdCustomer = {
            ...
            console.log(configIdCustomer)
            await this.$axios
              .get(`customer/list/`, configIdCustomer)
              .then(({ data }) => {
                this.customersList = []
        ...
        case 2:
          const configNameCustomer = {
            ...
            console.log(configNameCustomer)
            await this.$axios
              .get(`customer/list/`, configNameCustomer)
              .then(({ data }) => {
                this.customersList = []
                this.customersList = data.response   
              })
        ...
        case 3:
          const configCPFCustomer = {
            ...
            console.log(configCPFCustomer)
            await this.$axios
              .get(`customer/list/`, configCPFCustomer)
              .then(({ data }) => {
                this.customersList = []
                this.customersList = data.response   
              })
        ...
        case 4:
          const configEmailCustomer = {
            ...
            console.log(configEmailCustomer)
            await this.$axios
              .get(`customer/list/`, configEmailCustomer)
              .then(({ data }) => {
                this.customersList = []
                this.customersList = data.response   
              })
    ...
    generatePDF() {
      const columns = [
        ...
        { title: "Endereço", dataKey: "logradouro" },
        { title: "Sexo", dataKey: "sexo" },
        { title: "CPF", dataKey: "cpf" },
        { title: "Cidade", dataKey: "nome_cidade" },
        { title: "UF", dataKey: "uf" }
      ];
      ...
      try {
        ...
        await this.$axios
          ...
          .then(({ data }) => {
            this.editedItem[0] = {
              ...
              dt_nascimento: data.response[0].dt_nascimento,
              telefone: data.response[0].telefone,
              email: data.response[0].email,
              logradouro: data.response[0].logradouro,
              sexo: data.response[0].sexo,
              cpf: data.response[0].cpf,
            }
            this.dataCity = data.response[0].cod_cidade
          })
          .catch(err => {
            console.log('error on GET: ', err)
          })
        ...
        const configCity = {
          headers: {
            ...
            params: {
              id: parseInt(this.dataCity)
            }
        }

        await this.$axios
          .get(`city/list`, configCity)
          .then(res => {
            console.log("teste", res)
            this.editedItem[0].cidade = {
              codigo: res.data.response[0].codigo,
    ...
    async deleteItemConfirm (key) {
      ...
      await this.$axios
        .delete(`customer/delete/${key}`, {headers})
        .then(({data}) => {
          console.log(data)
          if(data.type == 'success') {
            ...
            alert('Este cliente possui atendimentos em seu nome, delete o atendimento e tente novamente!')
            this.closeDelete()
            return;
          }
        })
    ...
    save () {
      if (this.editedIndex > -1) {
        ...
        const data = {
          ...
          bornDate: this.editedItem[0].dt_nascimento,
        ...
        this.loading = true
        this.$axios
          .put(`customer/update/${this.editingCustomr}`, data, {headers})
          .then(({ data }) => {
            console.log(data)
            this.updateTable()
          })
         ...
         let token = Cookies.get('jwt-token')   
          ...
          this.loading = true
          this.$axios
            .post(`customer/store/`, data, {headers})
            .then(({ data }) => {
              console.log(data)
              this.updateTable()
            })
