...
class handleCity
{
    ...
    {
        return $this->City->getAllCities(self::TABLE);
    }
    ...
    private function filterByParams($data)
    {
        ...
        return $this->City->getCitiesByParams($param);
    }
    ...
    {
        return $this->City->getConn()->getOneByKey(self::TABLE, $this->data['id']);
    }
    ...
    private function store()
    {
        ...
        if ($description && $payed && $customerId) {
            if ($this->City->insertCity($description, $payed, $customerId) > 0) {
                $insertedId = $this->City->getConn()->getDb()->lastInsertId();
                $this->City->getConn()->getDb()->commit();
                return ['insertedId' => $insertedId];
            }

            $this->City->getConn()->getDb()->rollBack();
    ...
    {
        return $this->City->getConn()->delete(self::TABLE, $this->data['id']);
    }
    ...
    {
        if ($this->City->updateCity($this->data['id'], $this->bodyDataRequests) > 0) {
            $this->City->getConn()->getDb()->commit();
